            ...
            <div className='grid grid-cols-1 gap-7 lg:grid-cols-2  w-full'>
                {
                    blogPosts?.map(el => (
                        ...
                        <a href={`blog/${el.fields.slug}`} className='cursor-pointer w-full'> 
                            <div className='bg-white w-full shadow-md rounded-b-lg relative lg:flex' key={el.sys.id}>
                                <Image
                                ...
                                <div>
                                    <span className='absolute text-xs -mt-14 right-2 lg:right-2 lg:mt-0 lg:top-2 bg-blue-900  text-white rounded-xl px-4 py-1'>
                                        {dayjs(el.sys.createdAt).format('DD-MM-YYYY')}
                                    </span>
                                    <div className='px-5 py-5 mt-6'>
                                        <h2 className='font-bold text-sm'>{el.fields.title}</h2>
                                        <p className='text-xs'>{el.fields.description}</p>
                                    </div>
