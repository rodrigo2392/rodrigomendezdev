...
const options = {
    ...
    [BLOCKS.EMBEDDED_ASSET]: (node: any) => {
      ...
      [BLOCKS.PARAGRAPH]: (node: RenderNode, children: any) => <p className='mb-4 text-sm'>{children}</p>,
      [INLINES.HYPERLINK]: (node: any, children: any) => <a className='py-10 text-blue-600 cursor-pointer' href={node?.data?.uri} target='_bank'>{children}</a>,
    },
...
const Post = ({post}: Props) => {
    ...
    React.useEffect(()=> {
        setCurrentDate(dayjs(post.sys.createdAt).format('dddd D MMMM YYYY h:mm A'))
    }, [post.sys.createdAt])
    ...
    return (
        <div className='pb-10 w-full'>
            ...
            <h1 className='pl-5 text-left font-bold text-3xl  mt-2 md:pl-0'>{post.fields.title}</h1>
